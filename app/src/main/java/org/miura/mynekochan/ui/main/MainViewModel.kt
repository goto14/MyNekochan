...
class MainViewModel : ViewModel() {
    ...
    fun nextTurn() {
        ...
        while (newImg.size < maxCount * if (isAuto) autoAdj else 1) {
            val parents = newImg.filter { it.flag }.take(2)
            if (parents.size <= 1) {
                break
            }
            for (i in (0 until if (isAuto) autoRepeat else 1)) {
    ...
    private fun newBitmap(src1: Bitmap, src2: Bitmap): List<Bitmap> {
        ...
        for (i in (0 until h)) {
            for (j in (0 until w)) {
                ...
                val dice = r.nextInt(0 until 128)
                if (dice == 0) {
    ...
    fun autoDiff() {
        for (i in 0..10000) {
            ...
            val winnerCount = Math.min(images.size, maxCount * autoAdj / autoRepeat)
            val score = images.map { getScore(it.image!!) }
            val scoreIndex = score.indices.sortedByDescending { score[it] }
            val sorted = scoreIndex.map { images[it] }
            queue.clear()
            removedQueue.clear()
            sorted.take(winnerCount).forEach {
                it.flag = true
                it.deleted = false
            }
            sorted.drop(winnerCount).forEach {
                it.flag = false
                it.deleted = true
            }
            images = sorted
    ...
    private fun getScore(diff: Bitmap): Long {
        ...
        for (i in (0 until diff.height)) {
            for (j in (0 until diff.width)) {
                ...
                score -= abs(c1.red() - c2.red()).toInt() + abs(c1.green() - c2.green()).toInt() + abs(
                    c1.blue() - c2.blue()
                ).toInt()
            }
        }
